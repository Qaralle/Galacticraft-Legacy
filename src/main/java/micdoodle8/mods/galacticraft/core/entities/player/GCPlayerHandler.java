...
public class GCPlayerHandler {
	...
	public void onAttachCapability(AttachCapabilitiesEvent<Entity> event) {
		...
		} else if (event.getObject() instanceof EntityPlayer && event.getObject().world.isRemote) {
	...
	public void onPlayerUpdate(EntityPlayerMP player) {
		...
		if (tick % 250 == 0 && stats.getFrequencyModuleInSlot().isEmpty() && !stats.hasReceivedSoundWarning() && isInGCDimension && player.onGround && tick > 0 && ((IGalacticraftWorldProvider) player.world.provider).getSoundVolReductionAmount() > 1.0F) {
			...
			player.sendMessage(new TextComponentString(EnumColor.YELLOW + GCCoreUtil.translate("gui.frequencymodule.warning0") + " " + EnumColor.AQUA + GCItems.basicItem.getItemStackDisplayName(new ItemStack(GCItems.basicItem, 1, 19)) + sb));
